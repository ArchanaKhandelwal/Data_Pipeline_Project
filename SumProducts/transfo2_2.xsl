...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/Food">
    <!-- Select only the recipes of type "lunch" -->
    <xsl:apply-templates select="recipe[type='lunch']"/>
  </xsl:template>
  
  <!-- Template to calculate the sum for recipes of type "lunch" -->
  <xsl:template match="recipe[type='lunch']">
    <!-- Variable to store the sum -->
    <xsl:variable name="sum">
      <!-- Calculate the sum -->
      <xsl:call-template name="calculateSum">
        <!-- Pass all "lunch" recipe nodes as parameter -->
        <xsl:with-param name="recipes" select="."/>
      </xsl:call-template>
    </xsl:variable>
    
    <!-- Output the sum for recipes of type "lunch" -->
